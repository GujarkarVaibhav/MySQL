...
select sum(Purchase_cost), avg(estimated_sale_price) from Bank_inventory_pricing
where month=2;
...
select Product, count(*)from bank_inventory_pricing 
where month= 1 group by product having count(*)>1;


select Product, count(*)from bank_inventory_pricing 
group by product having month= 1 and count(*)>1;
...
sum(purchase_cost)-sum(ifnull(estimated_sale_price,0))
from 
bank_inventory_pricing
group by product 
having (sum(purchase_cost)>sum(ifnull(estimated_sale_price,0)))
and count(product)>1;
...
select sum(ifnull(purchase_cost,2000)) as total_cost from Bank_Inventory_pricing;

# Question 6:
# 6) Print unique records of bank_inventory_pricing without displaying the month.

select distinct Product,Quantity,purchase_cost,Estimated_sale_price from bank_inventory_pricing;
...
select Product,count(product) count from Bank_inventory_pricing
group by product;

# Question 9:
# 9) Print product and the difference of maximum and minimum purchase_cost of each product in Bank_Inventory_Pricing.

select product, (max(purchase_cost) -min(purchase_cost)) Difference from Bank_Inventory_Pricing
group by product ;

# Question 10:
# 10) Print the sum of Purchase_cost of Bank_inventory_pricing during 1st and 2nd month

select month ,sum(Purchase_cost) from Bank_inventory_pricing
where month in (1,2) group by month;

# Question 11:
# 11) Print Products with an average value of Purchase_cost per product only when exceeding average of 6000.

select product, avg(Purchase_cost) from Bank_inventory_pricing
group by product having avg(Purchase_cost)>6000;

# Question 12:
# 12) Print  products whose average of purchase_cost is less than sum of purchase_cost of  Bank_inventory_pricing.

select product,avg(purchase_cost),sum(purchase_cost) from Bank_inventory_pricing group by product 
...
select product,avg(Estimated_sale_price) from Bank_inventory_pricing
...
select product,max(estimated_sale_price) Max_estimated_sale_price from Bank_inventory_pricing
group by product;
...
select product,avg(estimated_sale_price)*1.15 Increased_estimated_sale_price,avg(purchase_cost),avg(estimated_sale_price) 
from Bank_inventory_pricing
group by product
...
# Question 18:
# 18) Forproduct = ‘BusiCard’,  print average of purchase_cost on condition that when purchase_cost  
# is not updated , choose any of the higher value between price  and estimated_sale_price

select product , 
avg(if(purchase_cost is not null,purchase_cost,if(price>estimated_sale_price,price,estimated_sale_price))) Actual_purchase_cost
from Bank_inventory_pricing
...
select product,avg(ifnull((estimated_sale_price),purchase_cost)) as Avg_estimated_sale_price
from Bank_inventory_pricing
group by product;
...
select grouped_product, avg(avg_price) 
from (
select product grouped_product, avg(price) avg_price ,month
from Bank_inventory_pricing
where month
group by product,month
) t
group by grouped_product
having count(grouped_product)>=3;
